...
#include "../../Component/ObjectBehavior/EnemyBehavior.h"
...
EnemyFactory::EnemyFactory(std::shared_ptr<AnimationData>& animData)
{
	for (int i = 0; i < maxMoveToPosEnemy; i++)
	{
		moveToPosBehaviorList_.emplace_back(std::make_shared<EnemyBehavior>(*this));
	}
...
void EnemyFactory::CreateMoveToPosEnemy(ObjectManager& objManager, const Math::Vector2& start, const Math::Vector2& end)
{
	...
	// ビヘイビアクラスを追加
	auto behavior = std::static_pointer_cast<EnemyBehavior>(std::move(moveToPosBehaviorList_.front()));
